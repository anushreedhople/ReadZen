...
-(void)authorizeUserWithClienID:(NSString *)client_ID
    ...
    NSLog(@"Inside authorizeUserWithClienID");
    ...
    if ([self checkIfAccessTokenInfoFileExists]) {
        NSLog(@"User access token file exists");
        ...
        NSLog(@"User access token file does not exist");
...
-(void)showWebviewForUserLogin{
    NSLog(@"Inside showWebviewForUserLogin");
    ...
    NSString *targetURLString = [NSString stringWithFormat:@"%@?scope=%@&redirect_uri=%@&client_id=%@&response_type=code&hd=apple.sch.ae", authorizationTokenEndpoint, scope, _redirectUri, _clientID];
    
    NSLog(@"targetURLString is %@",targetURLString);
...
-(void)removeAccessTokenInfoFile
{
    ...
    if(error != nil)
    {
        NSLog(@"Error occured file removing user access token info file");
    }
    else {
        NSLog(@"User access token file is removed");
    }
    [self logoutUser];
}
...
-(void) logoutUser {
    //Set the logout URL string
    NSString *logoutURLString = [NSString stringWithFormat:@"https://mail.google.com/mail/u/0/?logout&hl=en"];
    // Create and make a request based on the URL string.
    NSMutableURLRequest *request = [NSMutableURLRequest requestWithURL:[NSURL URLWithString:logoutURLString]];
    [self makeRequest:request];
}
